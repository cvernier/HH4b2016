...
void interpolation_normalization(bool flag_MMR=0){
    ...
    if (flag_MMR) {
        ...
        std::vector<double> mass_temp={400, 420, 450, 470, 500, 520, 550, 570, 600, 620, 650, 670, 700, 720, 750, 770, 800, 820, 850, 870, 900};
    ...
    for (unsigned i = 0; i<nPoints; i++) {
        ...
        gSystem->Exec(Form("cp -r %s_%d/w_data_Crystal.root %s_%d/w_data_Crystal.root", dir.c_str(), int(mass_0[lMarker]), dir.c_str(), int(mass[i])));
    ...
    TH1F* hr_2;
    if (flag_MMR) {
        hr_1 = c1->DrawFrame(350,0,1000,250);
        hr_1->SetTitle("MMR");
    } else {
    ...
    p_2->cd();
    if (flag_MMR) {
        hr_2 = c1->DrawFrame(350,-0.07,1000,0.07);
    } else {
    ...
    cout<<"end"<<endl;
    TCanvas *c_test = new TCanvas("canvas_test","canvas_test",800,800);
    h_test->Draw();
    if (flag_MMR) {
    ...
    TCanvas *c_test_1 = new TCanvas("canvas_test_1","canvas_test_1",800,800);
    h_test1->Draw();
