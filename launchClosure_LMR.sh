#!/bin/bash
  ...
  dest_dir_LMR="PreselectedWithRegressionDeepCSV/Closure/LMR/"
  seed=334477
  n_toys=300
 ...
 for signal_strenght in ${strenght_LMR[@]}
  do
          ...
          do
                  ...
                  combine datacard_${i}${background_LMR_1}.txt -M GenerateOnly -m $i -t ${n_toys} --saveToys -s ${seed} --expectSignal=$signal_strenght -
          ...
          root -l -b -q drawClosure_LMR.C\($signal_strenght,\"$name\",\"$background\"\)
          cd $DIR
  done
