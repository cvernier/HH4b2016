...
void BackgroundPrediction_AntiTag_CrystalBall(double plot_lo, double plot_hi, double rebin,
                                           ...
                                           std::string log="lin")
{
  ...
  TFile *f_data=new TFile("Histograms_BTag_all.root");
  TH1F *h_mX_SB=(TH1F*)f_data->Get(hist1.c_str());
  TH1F *h_mX_SR=(TH1F*)f_data->Get(hist2.c_str());
  std::cout<<h_mX_SB->GetEntries()<<std::endl;
  std::cout<<h_mX_SR->GetEntries()<<std::endl;	
  h_mX_SB->Rebin(rebin);
  h_mX_SR->Rebin(rebin);
  ...
  // SB Fit
  RooRealVar bC_p0("bC_p0", "bC_p0", crystalball_switch_lo, crystalball_switch_hi);
  RooRealVar bC_p1("bC_p1", "bC_p1", crystalball_exp_lo, crystalball_exp_hi);
  RooRealVar bC_p2("bC_p2", "bC_p2", crystalball_mean_lo, crystalball_mean_hi);
  RooRealVar bC_p3("bC_p3", "bC_p3", crystalball_width_lo, crystalball_width_hi);
  RevCrystalBall bC_fit("bC_fit", "Background Prediction PDF", *x, bC_p0, bC_p1, bC_p2, bC_p3);
  ...
  // bS
  RooRealVar bS_p0("bS_p0", "bS_p0", crystalball_switch_lo, crystalball_switch_hi);
  RooRealVar bS_p1("bS_p1", "bS_p1", crystalball_exp_lo, crystalball_exp_hi);
  RooRealVar bS_p2("bS_p2", "bS_p2", crystalball_mean_lo, crystalball_mean_hi);
  RooRealVar bS_p3("bS_p3", "bS_p3", crystalball_width_lo, crystalball_width_hi);
  RevCrystalBall bS_fit("bS_fit", "Background Prediction PDF", *x, bS_p0, bS_p1, bS_p2, bS_p3);
  ...
  double xPad = 0.0;//3;
  TCanvas *c_AntiTag_CrystalBall=new TCanvas("c_AntiTag_CrystalBall", "c_AntiTag_CrystalBall", 700*(1.-xPad), 700);	
  c_AntiTag_CrystalBall->SetFillStyle(4000);
  c_AntiTag_CrystalBall->SetFrameFillColor(0);
...
*/
  ...
  bC_plot->GetYaxis()->SetTitleFont(42);
  bC_plot->GetYaxis()->SetLabelFont(42);
  bC_plot->GetXaxis()->SetTitleFont(42);
  bC_plot->GetXaxis()->SetLabelFont(42);


  bC_plot->Draw();
  bS_plot->Draw("same");
  if (log=="log") c_AntiTag_CrystalBall->SetLogy();

  TPaveText *pave = new TPaveText(0.60, 0.60, 0.89, 0.69, "NDC");
  pave->SetBorderSize(0);
  pave->SetTextSize(0.027);
  pave->SetTextFont(42);
  pave->SetLineColor(1);
  pave->SetLineStyle(1);
  pave->SetLineWidth(2);
  ...
  leg->AddEntry(h_mX_SB, "Data shape in SB", "ep");
  h_mX_SR->SetLineColor(kBlue+1);
  h_mX_SR->SetMarkerColor(kBlue+1);  
  h_mX_SR->SetMarkerStyle(20);
  leg->AddEntry(h_mX_SR, "Data shape in SR", "ep");
  leg->Draw();

  CMS_lumi( c_AntiTag_CrystalBall, iPeriod, iPos );	
/*
  p_2->cd();
...
*/
  ...
  c_AntiTag_CrystalBall->SaveAs("c_AntiTag_CrystalBall.png");
  c_AntiTag_CrystalBall->SaveAs("c_AntiTag_CrystalBall.pdf");
