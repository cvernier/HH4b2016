...
void drawBias (const double signal_strenght=1.0, std::string Filename="", std::string background="") {
    ...
    for (int dd=0; dd!=1; ++dd) {
        ...
        for (int i=0; i!=nPoints; ++i) {
            ...
            TString name = Form("closure_%d_GeV-",mass[i])+dirNames[dd];
            TString name_mean = Form("closure_mean_%d_GeV-",mass[i])+dirNames[dd];
            TString name_sigma = Form("closure_sigma_%d_GeV-",mass[i])+dirNames[dd];
            TString title = Form("closure at %d GeV for ",mass[i])+dirNames[dd]+";#frac{#mu_{fit}-"+ftoa(signal_strenght)+"}{#sigma_{fit}};entries/0.25";
            TString title_mean = Form("closure at %d GeV for ",mass[i])+dirNames[dd]+";#mu_{fit}-"+ftoa(signal_strenght)+";entries/0.25";
            TString title_sigma = Form("closure at %d GeV for ",mass[i])+dirNames[dd]+";#sigma_{fit};entries/0.25";
            ...
            std::ofstream myfile;
            myfile.open(Form("latex_closure_%s%s.txt",(ftoa(signal_strenght)).c_str(), (background).c_str()), ios::app);
    ...
    c2->cd();
    leg->Draw("same");
    c2->SaveAs(Form(background+"__strenght_%s.pdf",(ftoa(signal_strenght)).c_str()));
    c2->SaveAs(Form(background+"__strenght_%s.png",(ftoa(signal_strenght)).c_str()));
    c2->SaveAs(Form(background+"__strenght_%s.root",(ftoa(signal_strenght)).c_str()));
    c2a->cd();
    leg2->Draw("same");
    c2a->SaveAs(Form(background+"_a_strenght_%s.pdf",(ftoa(signal_strenght)).c_str()));
    c2a->SaveAs(Form(background+"_a_strenght_%s.png",(ftoa(signal_strenght)).c_str()));
    c2a->SaveAs(Form(background+"_a_strenght_%s.root",(ftoa(signal_strenght)).c_str()));
    c3->cd();
    leg3->Draw("same");
    c3->SaveAs(Form(background+"__strenght_%s_sigma.pdf",(ftoa(signal_strenght)).c_str()));
    c3->SaveAs(Form(background+"__strenght_%s_sigma.png",(ftoa(signal_strenght)).c_str()));
    c3->SaveAs(Form(background+"__strenght_%s_sigma.root",(ftoa(signal_strenght)).c_str()));
    c3a->cd();
    leg4->Draw("same");
    c3a->SaveAs(Form(background+"_a_strenght_%s_sigma.pdf",(ftoa(signal_strenght)).c_str()));
    c3a->SaveAs(Form(background+"_a_strenght_%s_sigma.png",(ftoa(signal_strenght)).c_str()));
    c3a->SaveAs(Form(background+"_a_strenght_%s_sigma.root",(ftoa(signal_strenght)).c_str()));
