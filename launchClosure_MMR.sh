#!/bin/bash
  ...
  dest_dir_MMR="PreselectedWithRegressionDeepCSV/Closure/MMR/"
  seed=334477
  n_toys=300
 ...
 for signal_strenght in ${strenght_MMR[@]}
  do
          ...
          do
                  ...
                  combine datacard_${i}${background_MMR_1}.txt -M GenerateOnly -m $i -t ${n_toys} --saveToys -s ${seed} --expectSignal=$signal_strenght -
          ...
          root -l -b -q drawClosure_MMR.C\($signal_strenght,\"$name\",\"$background\"\)
          cd $DIR
  done
